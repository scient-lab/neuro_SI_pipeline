...
def gen_edge_input(max_dist, path, edge_feat):
    ...
    # Use a concrete NumPy int type instead of 'long'
    path_copy = path.astype(np.int32, order='C', casting='safe', copy=True)
    edge_feat_copy = edge_feat.astype(np.int32, order='C', casting='safe', copy=True)
    assert path_copy.flags['C_CONTIGUOUS']
    assert edge_feat_copy.flags['C_CONTIGUOUS']

    cdef numpy.ndarray[numpy.int32_t, ndim=4, mode='c'] edge_fea_all = -1 * np.ones(
